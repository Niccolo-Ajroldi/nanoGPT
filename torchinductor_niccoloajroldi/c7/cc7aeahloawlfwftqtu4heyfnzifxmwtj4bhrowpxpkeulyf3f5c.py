...
@pointwise(size_hints=[1024], filename=__file__, meta={'signature': {0: '*i64', 1: 'i32'}, 'device': 5, 'constants': {}, 'mutated_arg_names': [], 'configs': [instance_descriptor(divisible_by_16=(0, 1), equal_to_1=())]})
@triton.jit
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(5):
        torch.cuda.set_device(5) # no-op to ensure context
        buf0 = empty_strided((1, 1024), (1024, 1), device='cuda', dtype=torch.int64)
        stream5 = get_cuda_stream(5)
        triton__0.run(buf0, 1024, grid=grid(1024), stream=stream5)
        ...
        triton__1.run(buf3, buf6, primals_3, primals_1, buf0, primals_2, primals_4, buf1, buf5, buf7, 12288, 768, grid=grid(12288), stream=stream5)
        del buf3
        del primals_1
        del primals_2
...
if __name__ == "__main__":
    ...
    primals_1 = rand_strided((50304, 768), (768, 1), device='cuda:5', dtype=torch.float32)
    primals_2 = rand_strided((1024, 768), (768, 1), device='cuda:5', dtype=torch.float32)
    primals_3 = rand_strided((12, 1024), (1024, 1), device='cuda:5', dtype=torch.int64)
    primals_4 = rand_strided((768, ), (1, ), device='cuda:5', dtype=torch.float32)
